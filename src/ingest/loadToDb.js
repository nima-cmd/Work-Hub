...
export async function loadOrders(orders) {
  ...
  for (const o of orders) {
    ...
    await pool.query(
      `INSERT INTO orders
         (so_number, customer, po_number, is_ats, source, stage, so_status,
          qty_ordered, qty_allocated, qty_fulfilled, amount_paid, shipping_status,
          start_date, ship_date, cancel_date, notes, last_seen, last_movement, updated_at)
       ...
       ON CONFLICT (so_number) DO UPDATE SET
         ...
         updated_at      = now()`,
      [
        o.soNumber, o.customer || null, o.poNumber || null, o.isAts ?? null,
        ...
        o.cancelDate || null, o.notes || null,
      ],
    )
    n++
  }
...
export async function loadFulfillments(records) {
  let n = 0
  for (const r of records) {
    ...
    await pool.query(
      `INSERT INTO fulfillments
         (if_number, so_number, status, packed_status, days_pending, invoice_number, if_date, updated_at)
      ...
      [
        ...
        r.daysPending ?? null, r.invoice || null, r.date || null,
      ],
    )
    n++
  }
...
export async function loadInvoices(records) {
  let n = 0
  for (const r of records) {
    const inv = r.invoice
    ...
    await pool.query(
      `INSERT INTO invoices
         (inv_number, so_number, status, shipping_status, ship_date, updated_at)
       ...
       ON CONFLICT (inv_number) DO UPDATE SET
         ...
         updated_at      = now()`,
      [inv, so, r.soStatus || null, r.shippingStatus || null, r.shipDate || null],
    )
    n++
  }
  return n
...
export async function recordSnapshot(source, rowCount, fileModified = null) {
  await pool.query(
